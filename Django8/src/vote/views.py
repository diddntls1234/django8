...
#HttpRestponsRedirect : 클라이언트에게 HTML파일을 전달하는것이 아닌 
...
def detail(reqeust,question_id):
    ...
    #매개변수이름은 모델클래스에 작성한 변수를 사용가능
    #내부적으로 id변수(고유 키)가 추가됨
    obj = Question.objects.get(id=question_id)
    return render(reqeust, 'vote/detail.html',{'question':obj})
#투표 처리
def vote(request):
    ...
    if request.method == "POST":#클라이언트 요청이 POST인가?
        ...
        obj.votes += 1 # obj.vote = obj.vote + 1
        obj.save() #객체의 변경 사항을 데이터 베이스에 저장
        #obj.question : Choice 객체가 연결된 Question
        #obj.question.qestion_text : Choice 객체가 연결된 Question 객체에
...
def registerQ(request): 
    ...
    if request.method == "GET" :
        ...
        if form.is_valid(): #해당폼에 입력값들이 에러가 없는지 확인
            #form.save() 
            # 해당 폼에 입력값들로 모델클래스 객체를 생성 후 데이터 베이스에 저장 
            obj = form.save(commit=False)#해당 폼에 입력값들로 모델클래스 객체를 생성
            obj.pub_data = datetime.datetime.now()#현재시간을 대입
...
def registerC(request):
    if request.method =="GET":#vote/registerC.html을 사용 
        ...
        if form.is_valid():
            #print(request.user)
            #print(form.cleaned_data['question'].author)
...
def updateQ(request, question_id):
    ...
    if request.method == "GET":
        ...
        if form.is_valid():
            ...
            #question.pub_data = obj.pub_data
            #question.save()
